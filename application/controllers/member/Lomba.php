...
class Lomba extends CI_Controller {
	...
	public function __construct()
    {
            ...
            $this->load->model('member/model_lomba', 'dbObject');
    }
	...
	{   
        ...
        $this->load->view('member/lomba/tampil', $data);
	...
	{   
		...
		$this->load->view('member/templates/sidebar');
        $this->load->view('member/templates/footer');
        $this->load->view('member/lomba/tambah', $data);
        $this->load->view('member/templates/subfooter');
    ...
    public function tambah_proses(){
		...
		if ( ! $this->upload->do_upload('poster')){
            ...
            $lomba_deadline_tanggal=trim($this->input->post('lomba_deadline_tanggal'));
            $lomba_deadline_time=trim($this->input->post('lomba_deadline_time'));
			...
			$lomba_link=trim($this->input->post('lomba_link'));
			 $lomba_deadline=$lomba_deadline_tanggal." ".$lomba_deadline_time;
            ...
            if($this->dbObject->create_general( $this->lomba, $data)===TRUE)		// using direct parameter
			{
				...
				<script> 
					...
					location.replace("<?=base_url()?>index.php/member/lomba/"); 
				</script>
				<?php
				...
				<script> 
					...
					location.replace("<?=base_url()?>index.php/member/lomba/tambah"); 
				</script>
				<?php
				//redirect('kepegawaian/jabatan_insert','refresh');
			}
    }
    public function update_proses($param=""){
        $config['upload_path']          = './assets/img/poster';
		$config['allowed_types']        = 'gif|jpg|png';
		$config['max_size']             = (99999 * 9999);
		$config['max_width']            = (99999 * 9999);
		...
		$this->load->library('upload', $config);
        $poster= trim($this->input->post('poster_lama'));     
		if (  $this->upload->do_upload('poster')){
            $file ="assets/img/poster/".$poster;
    ...
    public function permintaan_lomba(){
		...
		$this->load->view('member/templates/sidebar');
        $this->load->view('member/templates/footer');
        $this->load->view('member/lomba/tampil_permintaan', $data);
        $this->load->view('member/templates/subfooter');
    }
    public function penentuan($param="", $param2=""){
        ...
        if($this->dbObject->update_general($this->lomba, 'lomba_id', $param, $data )===TRUE)		// using direct parameter
        {
            ...
            <script> 
                alert(" Gagal Proses. ");
                location.replace("<?=base_url()?>index.php/member/lomba/permintaan_lomba");   
            </script>
            <?php
    ...
    public function edit_lomba($param=""){
		...
		$this->load->view('member/templates/sidebar');
        $this->load->view('member/templates/footer');
        $this->load->view('member/lomba/edit', $data);
        $this->load->view('member/templates/subfooter');
